    ...
    static getProxyHandler(persistence) {
        return {
			...
			get : function(target, property, receiver) {
				...
				return Node.wrapRemoteAttribute(value, persistence);
			}
        }
    }
	
	static wrapRemoteAttribute(value, persistence) {
		if(Array.isArray(value)) {
			return value.map(entry => Node.wrapRemoteAttribute(entry, persistence));
		} else if(typeof value === 'object' && value != null) {
			return persistence.getNodesById(value.id, {}, 'all');
		} else {
			return value;
		}
	}
