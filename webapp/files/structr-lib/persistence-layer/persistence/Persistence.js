...
export class Persistence {
    ...
    async _persistObject(object) {
        ...
        if (object.id !== undefined && object.id !== null && object.type !== undefined && object.type !== null) {
            ...
            const response = await this._structrRest.post(object.type, Persistence._toSerializable(object));
			
			// Structr response must have result object.
			if (response.result === undefined) {
				throw new Error('Persistence: request yields no valid response');
			}
			
			result = response.result;
			
			// Only try to extract result, if it seems like there is a viable subresult in the post response
			if (result && result.length > 0) {
				if (result[0] && result[0].length > 0) {
					result = await this._structrRest.getById(object.type, result[0]);
				}
			}
        } else {
